...
def get_municipality_results(municipality_url):
    ...
    voters = envelopes = valid_votes = ""

    if summary_table:
        data_cells = summary_table.find_all('td', {'data-rel': 'L1'})
        if len(data_cells) >= 3:
            voters = data_cells[0].text.replace('\xa0', '').strip()
            envelopes = data_cells[1].text.replace('\xa0', '').strip()
            valid_votes = data_cells[3].text.replace('\xa0', '').strip()

    results = [voters, envelopes, valid_votes]

    party_tables = soup.find_all('table', {'class': 'table'})
    party_names = []
    votes_results = []

    for table in party_tables[1:]: # skip the first table with general info
    ...
    return results, party_names, votes_results
...
def main():
    ...
    first_results, party_names, _ = get_municipality_results(first_municipality)
